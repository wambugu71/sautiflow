...
class DLNAManager {
  ...
  Future<DeviceManager> start({reusePort = false}) async {
    ...
    _deviceManager = dm;
    // Android/Linux: another app may hold UDP 1900 without SO_REUSEADDR, making
    // the bind fail. Try reusePort first (supported there), then plain bind;
    // if both fail, continue without the multicast listener socket — discovery
    // still works via unicast M-SEARCH responses on the client socket.
    try {
      final canReusePort = Platform.isAndroid || Platform.isLinux || Platform.isMacOS;
      _socket_server = await RawDatagramSocket.bind(
        InternetAddress.anyIPv4,
        UPNP_PORT,
        reusePort: reusePort && canReusePort,
      );
    } on SocketException {
      try {
        _socket_server = await RawDatagramSocket.bind(
        ...
        print('dlna: could not bind SSDP port $UPNP_PORT ($e), '
            'continuing with M-SEARCH only');
        _socket_server = null;
      }
    }
