  ...
  void processFrame(String frameId, int size) {
    ...
    final handlers = switch (frameId) {
      ...
      "TPOS" || "TPA" => () {
          ...
          if (match != null) {
            metadata.discNumber = int.tryParse(match.group(1)!);
            metadata.totalDics = int.tryParse(match.group(2)!);
          } else {
      ...
      "TRCK" => () {
          ...
          if (match != null) {
            metadata.trackNumber = int.tryParse(match.group(1)!);
            metadata.trackTotal = int.tryParse(match.group(2)!);
          } else {
